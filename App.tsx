...
import { Search, GraduationCap, Library, Sparkles, AlertCircle, History, Clock, Trash2, ArrowRight, Plus } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [history, setHistory] = useState<HistoryItem[]>(() => {
    const saved = localStorage.getItem('scholar_history');
    return saved ? JSON.parse(saved) : [];
  });
  ...
  // Persist history
  useEffect(() => {
    localStorage.setItem('scholar_history', JSON.stringify(history));
  }, [history]);
      ...
      <aside className="w-80 bg-white border-r border-slate-200 flex flex-col hidden md:flex z-10 shadow-lg">
        ...
        <div className="flex border-b border-slate-100">
          <button 
            ...
            className={`flex-1 py-4 text-sm font-medium flex items-center justify-center transition-colors relative ${
              activeTab === 'bibliography' ? 'text-blue-600' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50'
            }`}
          >
                     ...
                     <div className="flex items-center justify-between text-xs text-slate-400 mt-2">
                       ...
                       <div className="flex items-center space-x-2">
                          <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium uppercase ${
                             item.language === 'en' ? 'bg-blue-50 text-blue-600' : 'bg-emerald-50 text-emerald-600'
                          }`}>
                            {item.language}
                          </span>
